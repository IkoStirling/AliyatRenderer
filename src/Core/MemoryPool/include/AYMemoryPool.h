...
namespace ayt::engine::pool::memory
{
	...
	*/
	...
	{
	public:
		MemoryPoolBase(size_t in_blockSize = 4096);
		~MemoryPoolBase();
	...
	class MemoryPoolBaseProxy : public Mod_MemoryPool
	{
	public:
		...
		static MemoryPoolBase& getMemoryPool(uint32_t index);
	...
	REGISTER_MODULE_CLASS("MemoryPool", MemoryPoolBaseProxy);
	...
	struct PoolDeleter {
		...
		void operator()(T* ptr) const {
			if (ptr) {
				...
				//	+ " :(" + std::to_string(i++) + ")\n";
				//std::cout << text;  // 日志
				MemoryPoolBaseProxy::destroyObject(ptr);  // 调用内存池的销毁逻辑
			}
		}
	};
	...
	template<typename T, typename... Args>
	T* MemoryPoolBaseProxy::createObject(Args&&... args)
	{
		T* p = nullptr;
		// 根据元素大小选取合适的内存池分配内存
		if ((p = reinterpret_cast<T*>(MemoryPoolBaseProxy::useMemoryPool(sizeof(T)))) != nullptr)
			// 在分配的内存上构造对象
	...
	template<typename T>
	void MemoryPoolBaseProxy::destroyObject(T* ptr)
	{
		if (ptr)
		{
			// 调用对象的析构函数
	...
	{
		return MemoryPoolBaseProxy::createObject<T>(std::forward<Args>(args)...);
	}

	template<typename T>
	void DeleteMObject(T* ptr)
	{
		MemoryPoolBaseProxy::destroyObject<T>(ptr);
	}
	...
	{
		T* p = MemoryPoolBaseProxy::createObject<T>(std::forward<Args>(args)...);
