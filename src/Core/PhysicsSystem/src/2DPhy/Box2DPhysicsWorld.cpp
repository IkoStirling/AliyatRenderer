...
namespace ayt::engine::physics
{
    ...
    void Box2DPhysicsWorld::setGravity(const math::Vector3& gravity)
    {
    ...
    void Box2DPhysicsWorld::setTransform(EntityID entity, const math::Transform& transform)
    {
    ...
    const math::Transform& Box2DPhysicsWorld::getTransform(EntityID entity)
    {
        auto it = _bodies.find(entity);
        if (it != _bodies.end())
        {
        ...
        static math::Transform emptyTransform;
    ...
    Box2DPhysicsWorld::GroundContactInfo Box2DPhysicsWorld::checkGroundContact(IPhysicsBody* body, float maxDistance)
    {
        ...
        // 获取碰撞体最低点
        math::Vector2 lowestPoint = box2DBody->getLowestPoint();
        ...
        if (result.hit)
        {
            ...
            info.contactPoint = math::Vector2(result.point.x, result.point.y);
            info.contactNormal = math::Vector2(result.normal.x, result.normal.y);
