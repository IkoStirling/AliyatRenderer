...
{
    Box2DPhysicsBody::Box2DPhysicsBody(b2WorldId worldId, const math::Vector2& position,
        float rotation, BodyType type)
    {
        b2BodyDef def = b2DefaultBodyDef();
        def.position = math::adapter::toBox2DV2(position);
        def.rotation = math::adapter::toBox2DRot(rotation);
    ...
    void Box2DPhysicsBody::setTransform(const math::Transform& transform) {
    ...
    math::Transform Box2DPhysicsBody::getTransform()
    {
        b2Transform trans = b2Body_GetTransform(_bodyId);
        return math::Transform{
            math::Vector3(trans.p.x, trans.p.y, 0),
            math::Vector3(0, 0, b2Rot_GetAngle(trans.q))
        };
    }

    math::Vector2 Box2DPhysicsBody::getPosition()
    {
        return math::adapter::fromBox2DV2(b2Body_GetPosition(_bodyId));
    }

    void Box2DPhysicsBody::setLinearVelocity(const math::Vector2& velocity) {
        b2Body_SetLinearVelocity(_bodyId, math::adapter::toBox2DV2(velocity));
    }

    math::Vector2 Box2DPhysicsBody::getLinearVelocity() const {
        b2Vec2 velocity = b2Body_GetLinearVelocity(_bodyId);
        return math::adapter::fromBox2DV2(velocity);
    }
    ...
    void Box2DPhysicsBody::applyForce(const math::Vector2& force) {
        b2Body_ApplyForceToCenter(_bodyId, math::adapter::toBox2DV2(force), true);
    }

    void Box2DPhysicsBody::applyImpulse(const math::Vector2& impulse) {
        b2Body_ApplyLinearImpulseToCenter(_bodyId, math::adapter::toBox2DV2(impulse), true);
    }
    ...
    // 碰撞查询
    void Box2DPhysicsBody::queryOverlapArea(const math::Vector4& area) {
        // 实现AABB查询逻辑
    }
    ...
    math::Vector4 Box2DPhysicsBody::getAABB() const
    {
        if (!B2_IS_NON_NULL(_bodyId))
            return math::Vector4(0.0f);
    ...
    std::vector<math::Vector2> Box2DPhysicsBody::getColliderVertices() const
    {
    ...
    math::Vector2 Box2DPhysicsBody::getLowestPoint() const
    {
        std::vector<math::Vector2> vertices = getColliderVertices();
        if (vertices.empty()) return math::Vector2(0);

        math::Vector2 lowestPoint = vertices[0];
    ...
    math::Vector2 Box2DPhysicsBody::getHighestPoint() const
    {
        std::vector<math::Vector2> vertices = getColliderVertices();
        if (vertices.empty()) return math::Vector2(0);

        math::Vector2 highestPoint = vertices[0];
        for (const auto& vertex : vertices)
        {
