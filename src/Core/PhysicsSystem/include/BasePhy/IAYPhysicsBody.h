...
namespace ayt::engine::physics
{
    ...
    public:
        ...
        //-----------------速度------------------
        virtual void setLinearVelocity(const math::Vector2& velocity) = 0;
        virtual math::Vector2 getLinearVelocity() const = 0;
        virtual void setAngularVelocity(float velocity) = 0;
        virtual float  getAngularVelocity() const = 0;


        //---------------力和运动----------------
        virtual void applyForce(const math::Vector2& force) = 0;
        virtual void applyImpulse(const math::Vector2& impulse) = 0;  //冲量
