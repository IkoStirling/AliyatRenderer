...
public:
    ...
    void setGravity(const AYMath::Vector3& gravity) override;

    bool raycast(const AYMath::Vector3& start, const AYMath::Vector3& end, STRaycastResult& hit);

    bool raycast(const AYMath::Vector3& start, const AYMath::Vector3& end,
        std::function<bool(const STRaycastResult&)> callback) override;

    IAYPhysicsBody* createBody(EntityID entity,
        const AYMath::Vector3& position,
        float rotation,
    ...
    struct GroundContactInfo
    {
        ...
        AYMath::Vector2 contactPoint;     // 接触点
        AYMath::Vector2 contactNormal;    // 接触法线
    ...
    // 检测特定点的地面碰撞
    STRaycastResult raycastToGround(const AYMath::Vector2& point, float maxDistance = 1.0f);
