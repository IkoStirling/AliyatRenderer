...
namespace ayt::engine::physics
{
    ...
    public:
        ...
        // 特有方法
        void setSize(const math::Vector2& size) override
        {
            _size = size;
            if (isValid()) {
    ...
    private:
        ...
        b2Polygon createBoxPolygon() const
        {
            ...
            // 如果有偏移或旋转，使用偏移版本的函数
            if (_offset != math::Vector2(0.0f, 0.0f) || _rotation != math::Vector3(0.0f)) {
                b2Vec2 center = { _offset.x, _offset.y };
                b2Rot rotation = math::adapter::toBox2DRot(_rotation.z);
                // 如果有圆角半径（如果需要的话）
