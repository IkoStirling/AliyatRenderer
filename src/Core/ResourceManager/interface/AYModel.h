...
public:
    ...
    // 获取模型数据
    const std::vector<STMesh>& getMeshes() const { return _meshes; }
...
private:
    ...
    void processMesh(const aiMesh* aiMesh, STMesh& mesh, const std::string& prefix) {
        // 处理顶点数据
    ...
    void processMaterial(const aiMaterial* aiMaterial, const std::string& prefix) {
        ...
        material.type = hasTransparency ? STMaterial::Type::Transparent : STMaterial::Type::Opaque;

        auto materialManager = GET_CAST_MODULE(AYMaterialManager, "MaterialManager");
