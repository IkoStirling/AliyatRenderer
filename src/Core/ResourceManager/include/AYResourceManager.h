...
namespace ayt::engine::resource
{
    ...
    */
    ...
    private:
        void _listenEvents();
        std::vector<std::unique_ptr<EventToken>> _tokens;
...
#include "AYResourceHandle.h"
    ...
    )
    {
        ...
        // 发布事件
        EventRegistry::publish(Event_ResourceLoadAsync<T, Args...>::staticGetType(),
            [request = std::move(request)](IEvent* event)  mutable {
