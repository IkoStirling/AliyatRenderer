    ...
    _preloadPath("@audios/AudioCachePreload.json")
{
    ...
    if (!device) {
        spdlog::error("[AYAVEngine] Failed to open OpenAL device");
        return;
    }

    ALCcontext* context = alcCreateContext(device, nullptr);
    if (!context) {
        spdlog::error("[AYAVEngine] Failed to create OpenAL context");
        alcCloseDevice(device);
        return;
    }

    if (!alcMakeContextCurrent(context)) {
        spdlog::error("[AYAVEngine] Failed to make OpenAL context current");
    ...
    if (system)
    {
        auto token = system->subscribe(Event_CameraMove::staticGetType(),
            [this](const IAYEvent& in_event) {
                ...
                //spdlog::info("[AYAVEngine] update sound location[{},{},{}]", tp.x, tp.y, tp.z);
...
void AYAVEngine::shutdown()
{
    ...
    _initialized = false;
    spdlog::info("[AYAVEngine] shutdown complete");
}
...
void AYAVEngine::_playNextTrack() {
    ...
    if (!playlist.currentAudio && attempts >= maxAttempts) {
        spdlog::error("[AYAVEngine] All tracks in playlist '{}' failed to play", _currentPlaylistName);
        _isPlaylistPlaying = false;
        _currentPlaylistName.clear();
    }
}
...
void AYAVEngine::_playAudioImpl(const std::shared_ptr<IAYAudioSource>& audio, const glm::vec3& position, bool loop, float volume, bool is3D, int slot)
{
    ...
    else
    {
        ...
        {
            spdlog::error("[AYAVEngine] Slot unavailable!");
            return;
        }
