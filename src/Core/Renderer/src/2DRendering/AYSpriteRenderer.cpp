...
namespace ayt::engine::render
{
    ...
    )
    {
        ...
        math::Matrix4 model = transform.getPixelSpaceMatrix(
            1,
            origin,
            size
        );
    ...
    void AYSpriteRenderer::_loadSpriteRendererConfigINI()
    {
        ...
        _baseVertex = _config.get<std::string>("shader path.base_vertex",
            Path::Engine::getPresetShaderPath() + std::string("SpriteRenderer/sprite.vert"));
        _atlasVertex = _config.get<std::string>("shader path.atlas_vertex",
            Path::Engine::getPresetShaderPath() + std::string("SpriteRenderer/sprite_atlas.vert"));
        _baseFragment = _config.get<std::string>("shader path.atlas_fragment",
            Path::Engine::getPresetShaderPath() + std::string("SpriteRenderer/sprite.frag"));
