...
namespace ayt::engine::render
{
	...
	void AYRendererManager::init()
	{
		...
		_device->setViewportCallback([this](int width, int height) {
			...
			getCameraSystem()->setViewportAll(math::Vector4(0, 0, framebufferWidth, framebufferHeight));
		...
		_renderer->getMaterialManager()->createMaterial(
			{
				.name = "1",
				.baseColor = math::Vector4(.2f, 0.5f, 0.1f, 1.0f),
				.metallic = 1.f,
				.roughness = 0.3f,
				.type = Material::Type::Opaque,
			}
			);
		_renderer->getMaterialManager()->createMaterial(
			{
				.name = "2",
				.baseColor = math::Vector4(.6f, 0.2f, 0.5f, 1.0f),
				.metallic = 0.3f,
				.roughness = 0.6f,
				.type = Material::Type::Opaque,
			}
		...
		_renderer->getMaterialManager()->createMaterial(
			{
				.name = "3",
				.baseColor = math::Vector4(.1f, 0.7f, 0.2f, 1.0f),
				.metallic = 0.8f,
				.roughness = 0.1f,
				.type = Material::Type::Opaque,
			}
		...
		lightManager->addDirectionalLight({
			.color = math::Vector3(1.0f, 0.95f, 0.9f),      // 暖白色
			.intensity = 1.2f,
			.direction = math::Vector3(-0.5f, -1.0f, -0.3f) // 左上后方照射
			});
		lightManager->addPointLight({
			.color = math::Vector3(0.8f, 0.9f, 1.0f),       // 冷色调
			.intensity = 0.8f,
			.position = math::Vector3(3.0f, 2.0f, 1.0f),
			.radius = 10.0f,
			.linear = 0.07f
			});
		lightManager->addSpotLight({
			.color = math::Vector3(1.0f, 0.9f, 0.7f),       // 暖黄色
			.intensity = 1.5f,
			.position = math::Vector3(0.0f, 3.0f, 2.0f),
			.direction = math::Vector3(0.0f, -0.7f, -0.5f), // 向下前方照射
		...
		_renderer->getUIRenderer()->createText(str, math::Vector3(2, 11, 0) * ppm, math::Vector4(1, 1, 1, 1), 1.f);
		_renderer->getUIRenderer()->createText(str, math::Vector3(2, 12, 0) * ppm, math::Vector4(1, 1, 1, 1), 1.f);
	...
	void AYRendererManager::_displayDebugInfo()
	{
		...
		_renderer->getFontRenderer()->renderText(fps, 25.0f, 25.0f, 1.0f, math::Vector3(1.0f, 1.0f, 1.0f));
		...
		for (int i = 1; i < 100; i++) {
			dr->drawRect2D({ math::Vector3(i * 10.f, 0, 0) }, math::Vector2(1.f), mat1, true);
			dr->drawRect2D({ math::Vector3(i * -10.f, 0, 0), math::Vector3(0, i * 10.f, 0) }, math::Vector2(0.5f), mat2, false);
		}
		for (int i = 1; i < 20; i++) {
			for (int j = 1; j < 20; j++) {
		...
		//	tex_ID,
		//	{ math::Vector3(0, 0, -0.9f) },
		//	//{},
		//	math::Vector2(0),
		//	math::Vector2(1),
		//	math::Vector2(10.0f),  // 大小
		//	math::Vector4(1.0f, 1.f, 1.f, 1.f),// 颜色
		//	false,
		//	false,
		...
		for (int i = 1; i < 100; i++) {
			_renderer->getCoreRenderer()->drawMesh(
				{
					math::Vector3(i * -5 + 250, 5, 5),
					math::Vector3(glm::radians(-90.f),0,0)
				},
				model->getMeshes()[0],
				false,
