...
namespace ayt::engine::render
{
    ...
    math::Matrix4 AY2DCamera::getViewMatrix() const
    {
        if (_dirtyView)
        {
    ...
    math::Matrix4 AY2DCamera::getProjectionMatrix() const
    {
        if (_dirtyProjection)
        {
            // 返回物理比例下的投影矩阵
    ...
    void AY2DCamera::setDeadzone(const math::Vector4& zone)
    {
        _deadzone = zone;
    }

    void AY2DCamera::setTargetPosition(const math::Vector2& targetPos)
    {
        _targetPosition = math::Vector3(targetPos, 0.f);
    }

    void AY2DCamera::setCurrentPosition(const math::Vector2& currentPos)
    {
