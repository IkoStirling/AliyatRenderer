...
{
	static int count = 0;
	spdlog::info("getProgram call: {}times", ++count);
	size_t delimiterPos = key.find('|');
	if (delimiterPos == std::string::npos) {
		spdlog::error("Invalid shader key format: {}", key);
...
const bgfx::Memory* AYBgfxCreator::loadMem(const bx::FilePath& _filePath)
{
	...
	spdlog::error("Failed to load {}.", _filePath.getCPtr());
	return NULL;
}
...
void* AYBgfxCreator::load(const bx::FilePath& _filePath, uint32_t* _size)
{
	...
	{
		spdlog::error("Failed to open: {}.", _filePath.getCPtr());
	}
