...
namespace ayt::engine::render
{
        ...
        const math::Vector4& color, float scale, bool is3D)
    {
        ...
        renderData.size = math::Vector3(1); // 文本大小动态计算
        renderData.color = color;
        renderData.texture = 0; // 使用字体纹理图集
        renderData.visible = true;
        renderData.dirty = true;
        renderData.scale = math::Vector3(1.0f);
        renderData.rotation = math::Vector3(0.0f);
    ...
    void AYUIRenderer::renderUI() {
        ...
        for (auto& batch : _uiBatches) {
            ...
            if (!batch.is3D)
            {
                ...
                model = math::Matrix4(1.f); //预留，不做实现
            }
            else
            {
            ...
            glUniform2fv(glGetUniformLocation(uiShader, "u_rectSize"), 1, glm::value_ptr(math::Vector2(66.7f)));
            ...
            glUniform4fv(glGetUniformLocation(uiShader, "u_fillColor"), 1, glm::value_ptr(math::Vector4(1, 1, 1, 1)));
            glUniform4fv(glGetUniformLocation(uiShader, "u_strokeColor"), 1, glm::value_ptr(math::Vector4(1, 0, 0, 1)));
    ...
    void AYUIRenderer::updateElementVertices(uint32_t elementIndex) {
        ...
        for (const auto& vertex : rectPoints) {
            ...
            math::Vector3 scaledVertex = math::Vector3(
            ...
            math::Vector3 adjustedPosition = renderData.position + math::Vector3(anchorOffsetX, anchorOffsetY, 0.0f);
            // 平移顶点到element的位置
            math::Vector3 transformedVertex = scaledVertex + adjustedPosition;
    ...
    void AYUIRenderer::updateTextVertices(uint32_t elementIndex) {
        ...
        for (char32_t c : utf32) {
            ...
            std::vector<math::Vector2> texcroods;
    ...
    void AYUIRenderer::_loadUIRendererConfigINI()
    {
        ...
        _uiVertexPath = _config.get<std::string>("shader path.ui_vertex",
            Path::Engine::getPresetShaderPath() + std::string("UIRenderer/ui.vert"));
