...
namespace ayt::engine::ecs
{
    ...
    class ECS : public Mod_ECSEngine
    {
    private:
        // 组件存储基础接口
    ...
    private:
        ...
        {
        public:
            View(ECS& world, std::vector<EntityID>&& entities)
            ...
            {
            public:
                Iterator(ECS& world, std::vector<EntityID>::iterator it)
                    : world(world), it(it) {
                }
    ...
    REGISTER_MODULE_CLASS("ECSEngine", ECS)

        template<typename T>
    inline ECS::ComponentPool<T>* ECS::getPool()
    {
        // 获取时如不存在初始化空池
    ...
    template<typename T>
    inline void ECS::ComponentPool<T>::add(EntityID entity, T&& component)
    {
    ...
    template<typename T>
    inline void ECS::ComponentPool<T>::remove(EntityID entity)
    {
    ...
    template<typename T>
    inline T& ECS::ComponentPool<T>::get(EntityID entity)
    {
        return components[entityToIndex.at(entity)];
    }

    template<typename T>
    inline bool ECS::ComponentPool<T>::has(EntityID entity)
    {
    ...
    template<typename T>
    inline T& ECS::getComponent(EntityID entity)
    {
        return getPool<T>()->get(entity);
    }

    template<typename T>
    inline bool ECS::hasComponent(EntityID entity)
    {
        auto type = std::type_index(typeid(T));
    ...
    template<typename TagType>
    inline void ECS::addTag(EntityID entity)
    {
