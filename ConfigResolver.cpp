...
namespace ayt::engine
{
	...
	public:
		...
		{
			std::string templatePath = Path::resolve(templateName);
	...
	std::vector<ConfigFile> scanConfigDirectory(const std::string& dirPath) {
		...
		for (const auto& entry : fs::recursive_directory_iterator(dirPath)) {
			if (entry.is_regular_file()) {
				...
				if (ext == ".ini") type = ConfigType::INI;
				else if (ext == ".json") type = ConfigType::JSON;
				else if (ext == ".xml") type = ConfigType::XML;
				else continue;
		...
		void handleFileAction(
			...
			try {
				...
				case efsw::Actions::Moved:
					...
					configFilesRef = scanConfigDirectory(Path::resolve("@config"));
	...
	// 获取配置文件类型
	ConfigType getConfigType(const std::string& filename) {
		std::string ext = fs::path(filename).extension().string();
		std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);

		if (ext == ".json") return ConfigType::JSON;
		if (ext == ".xml") return ConfigType::XML;
		if (ext == ".ini") return ConfigType::INI;
		return ConfigType::INI; // 默认
	}
	...
	int main()
	{
		...
		Configer resolverConfig;
		resolverConfig.loadFromFile("@config/ConfigResolver.ini", ConfigType::INI);
		...
		const std::string configDir = Path::resolve(std::string("@") + refConfigPath);
		...
		std::string staticPath = Path::resolve(std::string("@") + refWebStaticPath);
